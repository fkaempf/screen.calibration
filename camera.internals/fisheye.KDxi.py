...
# --- config ---
IMG_GLOB = 'calibration.pictures/1800 U 501m NIR-07XC0/automatic.chessboard/*.png'
SQUARE_SIZE = 1.0
PATTERNS = [(6, 9), (9, 6)]
TOP_N_BAD = 10
...
import os
os.makedirs("debug/reproj", exist_ok=True)
for it in per_file_sorted:
    ...
    out_path=f"debug/reproj/{os.path.basename(name)}_reproj.png"
)
...
os.makedirs("debug/rectified", exist_ok=True)
os.makedirs("debug/reproj", exist_ok=True)
...
for it in per_file_sorted:
    ...
    out_path=f"debug/rectified/{os.path.basename(name)}_rectified.png"
...
make_video("debug/rectified")
make_video("debug/reproj")
...
os.makedirs("debug/rectified.map", exist_ok=True)
for path in files_alt:
    ...
    cv2.imwrite(f"debug/rectified.map/{os.path.basename(path)}_rect.png", combo)
...
make_video_alt("debug/rectified.map")
